...
- **Ouvrages configurables** : un ouvrage de `OUVRAGES_DEFAUT` peut porter un objet `config` optionnel = `{ mode:'m2'|'forfait', dims:[{id,label,unit,default}], resume:(dims)=>string, base?:number|(dims)=>number, options:[{id,label,defaut,prix?}], exclusions?:string }`. À l'ajout au devis, `devis_view.js` ouvre un configurateur (dimensions + cases « ce qui apparaît dans le devis »). `composeConfig()` construit la ligne : description = `resume(dims)` + « Comprend : … » (options cochées) + `exclusions` ; prix `m2` = surface × prix/m², prix `forfait` = `base` + Σ prix des options cochées. Les ouvrages sans `config` gardent l'ajout en un clic. La description composée reste éditable (textarea) sur la ligne.
